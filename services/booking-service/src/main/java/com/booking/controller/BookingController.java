...
public class BookingController {
    ...
    public HttpResponse<?> create(@Body BookingRequest request, @Header("Authorization") String auth) {
        ...
        BookingDTO dto = new BookingDTO(booking.getId(), booking.getUserId(), booking.getTourId(),
                                        booking.getBookingDate(), booking.getStatus().name(),
                                        user.getName(), tour.getName());
    ...
    public HttpResponse<?> listByUser(@PathVariable Long userId, @Header("Authorization") String auth) {
        ...
        var dtos = bookings.stream().map(b -> {
            ...
            return new BookingDTO(b.getId(), b.getUserId(), b.getTourId(), b.getBookingDate(),
                                  b.getStatus().name(), user.getName(), tour.getName());
